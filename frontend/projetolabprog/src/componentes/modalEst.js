...
export default function ModalEst({idProducao, token}){
  ...
  function enviarEst(grad, mestrado, doutorado,idProducao){
     if(token == 'admin'){
    client.put(`attEstatisticasProd`, {qtdGraduado: grad,qtdMestrado: mestrado,qtdDoutorado: doutorado,idProducao: idProducao})
            .then(
                (response) => {
                ...
                alert("erro" + grad+" "+ mestrado+" "+ doutorado+" "+idProducao)
  ...
  <div className='Modal'>
      ...
      <from>
        ...
        <div class="to-do-form">					
          ...
          <button type="submit" onClick={() => {enviarEst(qtdGrad, qtdMest, qtdDout,idProducao); closePopup();} } >Salvar</button>						
        </div>
      </from>
    </div>
  </span> 
